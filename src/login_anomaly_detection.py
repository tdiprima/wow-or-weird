...
hours = np.random.choice(
    ...
    p=[
        ...
        0.01,
        0.01,
        0.02,  # 0-5am (people should be asleep)
        0.03,
        0.04,
        0.06,
        0.08,
        0.06,
        0.05,  # 6-11am (coffee-powered humans)
        0.06,
        0.05,
        0.04,
        0.04,
        0.05,
        0.06,  # 12-5pm (steady grind)
        0.08,
        0.07,
        0.06,
        0.04,
        0.03,
        0.02,  # 6-11pm (doomscroll hours)
    ],
)
...
if len(false_positives) > 0:
    ...
    fp_contributions = analyze_feature_contributions(
        fp_data, iso_forest, features, X_train_df
    )  # Added X_train_df

    print(
        f"Row: {df.loc[fp_idx][['hour_of_day', 'country', 'device_type', 'login_success', 'sessions_per_hour']].to_dict()}"
    )
...
if len(true_positives) > 0:
    ...
    tp_contributions = analyze_feature_contributions(
        tp_data, iso_forest, features, X_train_df
    )  # Added X_train_df

    print(
        f"Row: {df.loc[tp_idx][['hour_of_day', 'country', 'device_type', 'login_success', 'sessions_per_hour']].to_dict()}"
    )
...
plt.subplot(1, 3, 1)
plt.hist(
    test_df[test_df["true_anomaly"] == 0]["anomaly_score"],
    bins=30,
    alpha=0.7,
    label="Normal",
    color="green",
...
plt.hist(
    test_df[test_df["true_anomaly"] == 1]["anomaly_score"],
    bins=30,
    alpha=0.7,
    label="True Anomaly",
    color="red",
...
if len(false_positives) > 0:
    ...
    width = 0.35
    plt.bar(
        x - width / 2, normal_counts, width, label="Normal", alpha=0.7, color="green"
    )
    plt.bar(
        x + width / 2,
        fp_counts,
...
print(
    f"New login: {new_login[['hour_of_day', 'country', 'device_type', 'login_success', 'sessions_per_hour']].iloc[0].to_dict()}"
)
...
new_contributions = analyze_feature_contributions(
    new_X.values[0], iso_forest, features, X_train_df
)  # Added X_train_df
